  ...
  return (
    <div className="pb-10">
      {answers.map((answer: AnswerProp, i: number) => (
        <div
          key={i}
          className="w-full border border-amber-700 p-5 text-[14px]
        text-[#fff] mt-12 sm:mt-10 rounded-xl space-y-5"
        >
          ...
          <div className="flex justify-between items-center">
            <div className="flex justify-start items-center">
              ...
              {wallet === questionData?.owner && !questionData?.paidout && (
                <div className="flex items-center space-x-3 flex-wrap">
                  <div
                    className="flex space-x-1 border border-[#f53e3e] h-[32px] w-[90px]
                justify-center items-center rounded-full text-[#ee3333] cursor-pointer"
              ...
              {questionData?.winner === answer.owner && questionData?.paidout && (
                ...
                justify-center items-center rounded-full text-[#7afc38] cursor-pointer"
                  >
                    <FaEthereum className="w-[10px] h-[15px]" />
                    <p>Winner</p>
                  </div>
            ...
            <div className="space-x-2 hidden sm:flex items-center">
              <Identicon
                className="h-6 rounded-full bg-[#fff]"
                size={30}
                string={answer.owner}
              />
              <p className='text-[#fff]'>{truncate({ text: answer.owner, startChars: 4, endChars: 4, maxLength: 11 })}</p>
            </div>
          </div>
